...
import { HangulQuestionResult } from '../../data/types';
import { useLanguage } from '../../i18n';

interface HangulResultScreenProps {
  results: HangulQuestionResult[];
  onPlayAgain: () => void;
  onChangeSettings: () => void;
  onHome: () => void;
}

export default function HangulResultScreen({
  results,
  onPlayAgain,
  onChangeSettings,
  onHome,
}: HangulResultScreenProps) {
  const { t } = useLanguage();
          ...
          <div className={styles.missedGrid}>
            {missed.map((r, i) => (
              <div key={i} className={styles.missedItem}>
                <span className={styles.missedChar}>{r.question.character}</span>
