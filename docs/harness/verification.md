...
### ⛔ 검수 프롬프트를 보수적으로 유도하지 말 것 (2026-08-06 → 08-09 실측)

같은 데이터를 두 번 검수했는데 **1차 28건, 2차 163건**(6배)이 나왔다. 데이터가 바뀐 게
아니라 **검수 설계가 달랐다.** 1차가 놓친 이유:

| 요인 | 1차 (28건) | 2차 (163건) |
|---|---|---|
| 프롬프트 | "이미 검수를 거쳤으므로 **명백한 오류만** 남아 있을 가능성이 높다" | 그 문구 삭제, "한 항목도 건너뛰지 말 것" + 실측 사례 제시 |
| 청크 | 300개 | 250개 |
| 렌즈 | 한 에이전트가 4개 필드를 동시에 | 한국어는 **일본어 원어민 관점 / 한국어·영어 관점**으로 분리 |
| 교차 검사 | 없음 | 같은 표기·다른 읽기끼리 뜻이 같은지 기계 추출 후 집중 검사 |

**"이미 검수했다"는 정보를 검수자에게 주면 검출률이 급락한다.** 재검수 때는 그 사실을
숨기고, 대신 과거에 실제로 나온 오류 유형을 예시로 주어 무엇을 찾을지 알려줄 것.
